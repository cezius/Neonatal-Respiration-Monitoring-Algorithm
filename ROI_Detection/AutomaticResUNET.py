...
# After lets define a class for RESIDUAL layers
class Resid(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__() 
        self.resid_conv = nn.Sequential( 
            nn.Conv2d(in_channels, out_channels, kernel_size=5, padding=2),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True),
        )
    def forward(self, x):
        return self.resid_conv(x)
...
class Up(nn.Module):
    ...
    def forward(self, x1, x2, x3): # In the forward prop we have two inputs: one is coming from down (x1) and one
...
class UNET(nn.Module):
  def __init__(self, i_ch, o_ch):
    ...
    # residualconv
    self.resid1 = Resid(64,64)
    self.resid2 = Resid(128,128)
    self.resid3 = Resid(256,256)
    self.resid4 = Resid(512,512)
    #self.resid5 = Resid(1024,1024)
